...
then
    echo "Please provide an approximate focal length in pixels"
    exit 1
fi
...
GSPLAT_RESULT_PATH=$DATASET_PATH/gsplat_result
# Run splatting
python3 gaussian_splatting/train.py -s $DATASET_PATH --iterations 7000 -m $GSPLAT_RESULT_PATH
# Run sugar reconstruction
python3 train.py -s $DATASET_PATH -c $GSPLAT_RESULT_PATH -r density